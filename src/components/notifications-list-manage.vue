<template>
  ...
  >
    <h2 class="mb-4 flex items-center gap-2 text-lg">
      List of notifications
      <span class="badge">{{
        notificationsStore.getNotificationsByNamespace(namespace).length
      }}</span>
    </h2>

    <label class="label" for="notifications-list-namespace">Namespace</label>
    <select
    ...
    <ul class="flex flex-col gap-2">
      <li
        v-for="n of notificationsStore.getNotificationsByNamespace(namespace)"
        :key="n.id"
        class="flex justify-between gap-8 rounded-xl border border-neutral-200 bg-neutral-50 p-4"
      >
        <div class="badge badge-secondary w-20 shrink-0 grow-0">
          {{ n.type }}
        </div>
        <div class="grow">
        ...
        <div class="flex w-44 shrink-0 grow-0 items-start justify-end gap-4">
          <div class="flex items-center gap-2">
            <span class="badge badge-sm">{{ n.state }}</span>
            <button
...
function changeNotificationState(n: Notification) {
